...
JavaScript에서 Redux를 사용할 때는 상태가 바뀔 때마다 UI를 다시 그리도록 직접 `subscribe`를 연결합니다.
(React에서는ㄴ,,,)

### 🔹 상태 기반 렌더링

UI는 항상 **store.getState()의 값**으로 부터 만들어집니다.
...
버튼 클릭 같은 이벤트는 상태를 직접 바꾸지 않고 항상 **action을 dispatch**합니다.
...
1. state가 undefined일 때 → 초기 상태 반환
2. ㅁction.type에 따라 분기
3. 기존 state를 변경하지 않고 새로운 state생성
4. 새로운 객체를 return
